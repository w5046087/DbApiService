...
using DpApiServer.Core.Common.Http;

namespace DpApiServer.Core.Common.Wx
{
    public class WxHandle
    {
        private readonly ISend sendRequest;

        private readonly IMemoryCache _memoryCache;

        public WxHandle(ISend send, IMemoryCache memoryCache)
        {
            sendRequest = send;
        ...
        public async Task<RsMsgSimple> GetWxAccessToken()
        {
            ...
            {
                var response = await sendRequest.GetAsync(Url_AccessToken);
                var result = await response.Content.ReadFromJsonAsync<Model.AccessTokenModel>();
        ...
        //        var response = await sendRequest.PostAsync(String.Format(Url_SendTemplateMsg, token), new StringContent(jsonData, Encoding.UTF8, "application/json"));
